...
class MessageProcessor:
    ...
    async def process_message(self, event: Dict) -> Optional[int]:
        ...
        try:
            ...
            message_data = {
                ...
                "images": [
                    {"file_url": img.file_url, "file_unique": img.file_unique, "file_size": img.file_size}
            ...
            self.logger.info(
                format_stored_log(event, parsed, self.settings.forward.max_depth, message_db_id)
            )
    ...
    async def _process_forwards(self, forward_ids: list[str]) -> list[dict]:
        ...
        for forward_id in forward_ids:
            ...
            try:
                ...
                nodes = parse_forward_response(response, max_depth=self.settings.forward.max_depth)
    ...
    async def _process_images(self, message_db_id: int, images_info: list[ImageInfo]) -> None:
        ...
        if not self.settings.image.download:
            return
        try:
            results = await process_images(images_info, self.settings.storage.images_dir, self.settings.image)
