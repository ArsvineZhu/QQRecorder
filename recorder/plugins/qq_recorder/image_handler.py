...
def detect_extension(url: str = "", content_type: str = "", image_data: bytes = b"") -> str:
...
def generate_filename(file_unique: str, url: str = "", content_type: str = "", image_data: bytes = b"") -> str:
    base = file_unique if file_unique and file_unique != "0" else f"{int(datetime.datetime.now().timestamp()):x}_{os.urandom(4).hex()}"
    ...
    try:
        ...
        async with session.get(url) as response:
            ...
            if content_length and int(content_length) > max_size:
                raise ImageDownloadError(f"File too large: {content_length} bytes > {max_size} bytes")
            data = await response.read()
            if len(data) > max_size:
                raise ImageDownloadError(f"File too large: {len(data)} bytes > {max_size} bytes")
...
async def process_image(image_info: ImageInfo, config_storage_dir: str, config_image: ImageConfig, session: aiohttp.ClientSession = None) -> ImageResult:
    try:
        ...
        filename = generate_filename(md5_hash, image_info.file_url, content_type, image_data)
        ...
        return ImageResult(local_path="", file_unique="", file_size=0, success=False, error=str(e))
    except Exception as e:
        return ImageResult(local_path="", file_unique="", file_size=0, success=False, error=f"Processing failed: {str(e)}")

async def process_images(images: List[ImageInfo], config_storage_dir: str, config_image: ImageConfig) -> List[ImageResult]:
    results = []
    if not config_image.download:
        for img in images:
            results.append(ImageResult(local_path="", file_unique=img.file_unique, file_size=img.file_size, success=False, error="Download disabled"))
        return results
    timeout_obj = aiohttp.ClientTimeout(total=config_image.timeout)
    async with aiohttp.ClientSession(timeout=timeout_obj) as session:
        for img in images:
            if not img.file_url:
                results.append(ImageResult(local_path="", file_unique=img.file_unique, file_size=img.file_size, success=False, error="Empty URL"))
                continue
            result = await process_image(img, config_storage_dir, config_image, session=session)
