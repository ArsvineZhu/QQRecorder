...
```
NcatBot Event → plugin._event_to_dict() → _process_message()
...
|------|------|-------------|
| `plugin.py` | Event handlers, command routing, orchestration | `QQRecorderPlugin` |
...
- **Plugin entry**: `main.py` re-exports from `plugin.py`. NcatBot loads via `manifest.toml` → `entry_class = "QQRecorderPlugin"`.
...
- **Per-message error isolation**: `_process_message()` wraps everything in try/except — one bad message never kills the plugin.
...
- **NEVER** modify `event` objects in-place — convert to dict first via `_event_to_dict()`

## GOTCHAS

- `ImageInfo.file_unique` is often `"0"` from QQ — don't rely on it for dedup. Use MD5 hash instead.
- `_event_to_dict()` manually extracts fields because NcatBot event objects don't serialize cleanly.
