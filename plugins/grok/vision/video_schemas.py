...
logger = logging.getLogger("grok.vision.video_schemas")
...
def normalize_video_analysis(
    ...
    key_events = adapted.get("key_events", []) or []
    if isinstance(key_events, list):
        for item in key_events:
            ...
            if isinstance(time_range, dict):
                item["time_range"] = _flatten_time_range(time_range)

    aff = adapted.get("affective_reading", {}) or {}
    unc = adapted.get("uncertainty", {}) or {}
    saf = adapted.get("safety_and_privacy", {}) or {}
    ...
    events = []
    for item in adapted.get("key_events", []) or []:
        if isinstance(item, dict):
            events.append(
                KeyEvent(
    ...
    analysis = VideoAnalysis(
        media_type="video",
        video_type=str(adapted.get("video_type", "unknown") or "unknown"),
        duration_summary=str(adapted.get("duration_summary", "") or ""),
        visual_summary=str(adapted.get("visual_summary", "") or ""),
        key_events=events,
        visible_text=_coerce_string_list(adapted.get("visible_text", [])),
        audio_or_speech_summary=str(adapted.get("audio_or_speech_summary", "") or ""),
        semantic_meaning=str(adapted.get("semantic_meaning", "") or ""),
        contextual_meaning=str(adapted.get("contextual_meaning", "") or ""),
        ...
        confidence=_clamp_float(adapted.get("confidence", 0.0)),
        raw_model_output=raw_model_output,
    )

    discarded = DISCARD_VIDEO_DECISION_FIELDS & adapted.keys()
    if discarded:
...
def _flatten_time_range(payload: dict[str, Any]) -> str:
    start_time = _clean_time(payload.get("start_time") or payload.get("start"))
    end_time = _clean_time(payload.get("end_time") or payload.get("end"))
