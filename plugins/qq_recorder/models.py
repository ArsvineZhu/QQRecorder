...
class Message(Base):
    ...
    segments: Mapped[list["MessageSegment"]] = relationship(
        "MessageSegment", back_populates="message", cascade="all, delete-orphan"
    )
    ...
    forward_messages: Mapped[list["ForwardMessage"]] = relationship(
        "ForwardMessage", back_populates="message", cascade="all, delete-orphan"
    )
...
class ForwardMessage(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    message_id: Mapped[int] = mapped_column(ForeignKey("messages.id"))
    parent_forward_id: Mapped[int | None] = mapped_column(
        ForeignKey("forward_messages.id"), nullable=True
    )
    ...
    message: Mapped["Message"] = relationship(
        "Message", back_populates="forward_messages"
    )
    children: Mapped[list["ForwardMessage"]] = relationship(
        "ForwardMessage", back_populates="parent", cascade="all, delete-orphan"
    )
    parent: Mapped["ForwardMessage | None"] = relationship(
        "ForwardMessage", back_populates="children", remote_side=[id]
    )
