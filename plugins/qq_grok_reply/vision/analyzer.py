...
logger = logging.getLogger("qq_grok_reply.vision.analyzer")
...
async def analyze_image(
    ...
    *,
    ...
    """Analyze a single image via DashScope vision model.

    Args:
        client: DashScope OpenAI-compatible client.
        image_base64: Base64-encoded image data (without data URI prefix).
        file_unique: MD5 hash of the image (for logging/caching).
        settings: Plugin settings (used to read vision config).
        chat_context: Lightweight text context for the vision model.
        model_override: Optional model name override (used by router).
            If None, uses settings.vision.image_fast_model.

    Returns:
        Normalized VisualAnalysis. On failure, returns an analysis with
        ``error_code`` set.
    """
    ...
    # Inject image into the user message's content list
    user_msg = messages[1]
...
async def _call_vision_model(
    ...
    """Run the vision chat completion in a thread to avoid blocking the event loop."""
