"""Video analysis: call DashScope video model, parse and normalize response.

Only analyzes video when the message triggers the bot (prefix/@/reply_to_bot).
"""
...
logger = logging.getLogger("qq_grok_reply.vision.video_analyzer")
...
async def analyze_video(
    ...
    *,
    ...
    """Analyze a single video via DashScope vision model.

    Args:
        client: DashScope OpenAI-compatible client.
        video_file_path: Local path to the video file (if available).
        video_url: Direct URL to the video (if available).
        file_unique: MD5 of the video (for logging/caching).
        settings: Plugin settings.
        chat_context: Lightweight text context.
        title: Video title.
        intro: Video intro/description.

    Returns:
        Normalized VideoAnalysis. On failure, returns analysis with
        ``error_code`` set.
    """
